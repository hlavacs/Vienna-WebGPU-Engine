...
{
	DebugPrimitive p;
	p.type = static_cast<uint32_t>(DebugPrimitiveType::Line);
	p.color = color;
...
{
	DebugPrimitive p;
	p.type = static_cast<uint32_t>(DebugPrimitiveType::Disk);
	p.color = color;
...
{
	DebugPrimitive p;
	p.type = static_cast<uint32_t>(DebugPrimitiveType::AABB);
	p.color = color;
	p.data.aabb.min = min;
	p.data.aabb.max = max;
	return p;
...
{
	DebugPrimitive p;
	p.type = static_cast<uint32_t>(DebugPrimitiveType::Arrow);
	p.color = color;
