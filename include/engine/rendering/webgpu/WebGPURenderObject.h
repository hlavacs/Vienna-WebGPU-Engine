...
namespace engine::rendering::webgpu
{
	...
	public:
			  ...
			  m_lastSyncedVersion(0)
		{
			...
			static_assert(std::is_base_of_v<engine::core::Identifiable<CPUObjectT>, CPUObjectT>,
						  "CPUObjectT must derive from engine::core::Identifiable<CPUObjectT>");

			// Static assert that CPUObjectT::Handle is the same as Handle<CPUObjectT>
			static_assert(std::is_same_v<typename CPUObjectT::Handle, engine::core::Handle<CPUObjectT>>,
						  "CPUObjectT::Handle must be engine::core::Handle<CPUObjectT>");

			// Static assert that CPUObjectT implements Versioned
			static_assert(std::is_base_of_v<engine::core::Versioned, CPUObjectT>,
						  "CPUObjectT must derive from engine::core::Versioned");
		}
